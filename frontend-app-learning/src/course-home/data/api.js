...
//================================= Custom for PP2 ====================================
export async function getSubmitTabData(courseId, username) {
  let url_get_email = `${getConfig().LMS_BASE_URL}/api/user/v1/accounts?usernames={${username}}`;
  console.log(`getSubmitTabData...url_get_email: ${url_get_email}`)
  let email = ''
  try {
    const { data } = await getAuthenticatedHttpClient().get(url_get_email);
    const camelCasedData = camelCaseObject(data);
    email = camelCasedData[0].email;
    return {}
  }catch (error) {
    const { httpErrorStatus } = error && error.customAttributes;
    if (httpErrorStatus === 404) {
